...
export function ReviewPanel() {
  ...
  return (
    <aside className="flex flex-col gap-1 rounded-2xl bg-review-bg p-6 lg:sticky lg:top-6">
      ...
      <div className="mt-2 flex items-end justify-between gap-4">
        <div className="flex size-20 shrink-0 flex-col items-center justify-center rounded-full border-4 border-dashed border-white bg-brand p-2 text-center text-[10px] leading-tight font-bold text-white">
          {guarantee.label}
        </div>
        <div className="flex flex-col items-end gap-1">
          <span className="mb-1 rounded-full bg-brand-dark px-3 py-1 text-xs font-semibold text-white">
          ...
          <span className="text-2xl font-bold text-foreground">{formatMoney(totals.subtotal)}</span>
        </div>
      </div>
      ...
      <button
        ...
        className="mt-4 rounded-full bg-brand py-3 text-center text-sm font-semibold text-white transition-colors hover:bg-brand-dark"
      >
      ...
      <button
        type="button"
        onClick={handleSave}
        className="mt-2 text-center text-sm font-medium text-muted-foreground underline underline-offset-2"
      >
